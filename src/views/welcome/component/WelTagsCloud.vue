	...
	<div class="page">
		<div class="tagsCloud" :style="{ height: boxWidth + 'px', width: boxWidth + 'px' }" ref="Panel" @mousemove="listener($event)">
			<div
				...
				:key="index"
				:style="[
					{ opacity: (400 + item.z) / 500 },
					{ 'font-size': `${12 * ((item.count * 0.0001)) < 24 ? 12 * ((item.count * 0.0001)) : 24 }` + 'px' },
					{ left: item.x + 'px' },
					{ top: item.y + 'px' },
	...
	methods: {
		init() {
			...
			this.useArray.forEach((item, key) => {
				...
				// X,Y,Z
				this.$set(item, 'x', this.radius * Math.sin(a) * Math.cos(b) + this.CX);
				this.$set(item, 'y', this.radius * Math.sin(a) * Math.sin(b) + this.CX);
				this.$set(item, 'z', this.radius * Math.cos(a));
				...
				if (this.randomColor) {
					...
					// this.$set(item, 'rcolor', 'rgb(' + parseInt(Math.random() * 255) + ',' + parseInt(Math.random() * 255) + ',' + parseInt(Math.random() * 255) + ')');
					this.$set(item, 'rcolor',keyWOrdColor[ Math.floor(Math.random()*(6-0+1)+0)]);
				}

				// Z-index
				let scale = this.boxWidth / (this.boxWidth - item.z);
				this.$set(item, 'zindexVal', parseInt(scale * 100));
			});
			// console.log('----------------------------')
		},
		rotateX() {
