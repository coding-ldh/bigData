          ...
          <template #header>
            <el-form :inline="true" class="demo-form-inline">
              ...
              <el-form-item>
                <el-select
                  placeholder="请选择场景"
                  v-model="overviewInfo.scene"
                >
                  <el-option value="" label="">全部</el-option>
                  <el-option
                    v-for="item in overviewScenes"
                    :key="item"
                    :value="item"
                  ></el-option>
                </el-select>
          ...
          <template #default>
            ...
            >
              ...
              <el-form-item>
                <el-select
                  placeholder="请选择场景"
                  v-model="sellModelInfo.scenes"
                  @change="handleSellModel"
                  multiple
                >
                  <el-option value="" label="">全部</el-option>
                  <el-option
          ...
          <template default>
            ...
            >
              ...
              <el-form-item>
                <el-select
                  placeholder="请选择场景"
                  v-model="deviceModelInfo.scenes"
    ...
    <el-row :gutter="24">
      ...
      >
        <el-card>
          <template #header>
            <span>语音对话数据</span>
            <tool-tip :content="aiTip"></tool-tip>
          </template>
          <template default>
            <div style="height: 500px">
              <div class="ai_info">
                <div class="y_ai_count">
                  <div class="title" style="margin-bottom: 14px">
                    昨日对话数量（次数）<span class="count">{{
                  ...
                  <div class="compare">
                    ...
                    <svg-icon
                      :icon-class="
                        aiVoiceInfo.yesterdayVoiceCountAdd > 0
                          ? 'welcome4'
                          : 'welcome5'
                      "
                    />
                    <span class="count">
                      {{ aiVoiceInfo.yesterdayVoiceCountAdd }}
                    </span>
                  </div>
                </div>
                <div class="t_ai_count">
                  <span class="title"> 今日实时对话次数：</span
                  ><span class="count">{{ aiVoiceInfo.todayVoiceCount }}</span>
                </div>
              </div>
              <div
                class="bar"
              ...
              >
                <div class="total_count">
                  <div class="title">累计对话数量(次数)</div>
                  <div class="count">{{ aiVoiceInfo.totalVoiceCount }}</div>
                </div>
                <div class="total_count">
                  <div class="title">累计有效配置对话数量(次数)</div>
                  <div class="count">
                    {{ aiVoiceInfo.totalValidVoiceCount }}
                  </div>
                </div>
                <div class="total_count">
                  <div class="title">累计有效配置比例</div>
                  <div class="count">{{ aiVoiceInfo.validVoiceRate }}%</div>
                </div>
              </div>
            </div>
          </template>
          ...
          <template #default>
            <div class="general_flex">
              <el-form :inline="true" class="demo-form-inline">
                <el-form-item>
                  <ReArea
                    @fromChildArea="getSceneAreaData"
                    ref="areaRef"
                    :handleArea="handleArea"
                  ></ReArea>
                </el-form-item>
              </el-form>
              <el-button type="primary" @click="exportScene">导出</el-button>
            </div>
            <el-table
              style="width: 100%"
              :data="sceneList"
              ...
              <el-table-column label="场景名称" align="center">
                <template slot-scope="scope">
                  <el-button
                    ...
                    >{{ scope.row.projectScene }}</el-button
                  >
                </template>
              </el-table-column>
              <el-table-column label="设备总量/占比" align="center">
                <template slot-scope="scope">
                  {{ scope.row.robotTotal }}/{{ scope.row.robotTotalRate }}%
                </template>
              </el-table-column>
              <el-table-column
                label="昨日/近7日/近30日新增激活设备"
                align="center"
              >
                <template slot-scope="scope">
                  {{ scope.row.increRobotYesterday }}/{{
                    scope.row.curRobot7d
                  }}/{{ scope.row.curRobot30d }}
                </template>
              </el-table-column>
              <el-table-column label="有效项目总量/占比" align="center">
                <template slot-scope="scope">
                  {{ scope.row.projectValid }}/{{ scope.row.projectValidRate }}%
                </template>
              </el-table-column>
              <el-table-column label="项目总量/占比" align="center">
                <template slot-scope="scope">
                  {{ scope.row.projectTotal }}/{{ scope.row.projectTotalRate }}%
                </template>
              </el-table-column>
              <el-table-column label="昨日/近7日/近30日新增项目" align="center">
                <template slot-scope="scope">
                  {{ scope.row.increProjectYesterday }}/{{
                    scope.row.curProject7d
                  }}/{{ scope.row.curProject30d }}
                </template>
              </el-table-column>
            </el-table>
          </template>
          ...
          <template default>
            ...
            >
              ...
              <el-table-column
                ...
                align="center"
              ></el-table-column>
              <el-table-column label="主要场景/占比" align="center">
                <template slot-scope="scope">
                  {{ scope.row.projectScene }}/{{ scope.row.scenePercent }}%
                </template>
              </el-table-column>
            ...
            <div class="general_flex">
              <el-form :inline="true" class="demo-form-inline">
                ...
                <el-form-item>
                  <el-select
                    placeholder="请选择场景"
                    v-model="top10ProjectInfo.scene"
                    @change="handleTop10ProjectList"
                  >
                    <el-option value="" label="">全部</el-option>
                    <el-option
                      v-for="item in overviewScenes"
...
export default {
  ...
  methods: {
    ...
    handleSelect(item) {
      // console.log(this.sellModelInfo);
    },
    ...
    async setOverviewBase() {
      const { data } = await getOverviewBase();
    ...
    async setAiVoiceInfo() {
      ...
      this.aiVoiceInfo.yesterdayVoiceCountAdd = data.yesterdayVoiceCountAdd;
    ...
    async handleOverview() {
      ...
      // console.log(this.overviewTable.list);
    ...
    exportData() {
      ...
      const url = `/projectoverview/overview/trend/export?beginDate=${beginDate}&endDate=${endDate}&region=${region}&province=${province}&city=${city}&scene=${scene}&hardware=${hardType}&software=${softType}&version=${version}&fileName=概览数据`;
      userExport(url);
    },
    exportScene() {
      const { region, province, city } = this.sceneAdress;
      const address = "-" + (city ? city : province ? province : region);
      const fileName = `项目概览${address}-场景分布`;
      const url = `/projectoverview/scene/export?region=${region}&province=${province}&city=${city}&fileName=场景分布`;
      userExport(url, fileName);
    },
    exportProvince() {
      const { region, province, software, hardware } = this.top10ProvinceAdress;
      const address = "-" + (province ? province : region);
      const fileName = `项目概览${address}-Top10省份`;
      const url = `/projectoverview/top10province/export?region=${region}&province=${province}&software=${software}&hardware=${hardware}&fileName=top10省份`;
      userExport(url, fileName);
    },
    exportProject() {
      const { region, province, city, scene } = this.top10ProjectInfo;
      // console.log(this.top10ProjectInfo);
      const address = "-" + (city ? city : province ? province : region);
      const fileName = `项目概览${address}-Top10项目`;
    ...
    exportCity() {
      const { region, province, city, software, hardware } =
        this.top10CityAdress;
      const address = "-" + (city ? city : province ? province : region);
      const fileName = `项目概览${address}-Top10城市`;
      const url = `/projectoverview/top10city/export?region=${region}&province=${province}&city=${city}&software=${software}&hardware=${hardware}&fileName=top10城市`;
      userExport(url, fileName);
    },
